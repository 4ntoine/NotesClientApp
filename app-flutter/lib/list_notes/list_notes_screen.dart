...
class ListNotesScreen extends StatefulWidget {
  ...
  ListNotesScreen(this._useCase, this._addNoteCallback, {Key key})
      : super(key: key);
...
class _ListNotesScreenState extends State<ListNotesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final refreshIcon =
        Platform.isAndroid ? Icons.refresh : CupertinoIcons.refresh;
    final double topMargin = Platform.isAndroid ? 0 : 85;

    return Platform.isAndroid
        ? Scaffold(
            appBar: AppBar(title: Text('Notes list'), actions: <Widget>[
              IconButton(
                  icon: Icon(addIcon), onPressed: widget._addNoteCallback),
